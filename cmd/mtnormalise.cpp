...
// std::cout << "M and alpha set" << std::endl;
   M = X.transpose()*X;
   alpha = X.transpose()*y;
// std::cout << "Ready to do llt().solve" << std::endl;
...
*/
 ...
 // return num_voxels;
// ThreadedLoop(mask,0,3).run(MaskModification(num_voxels, lower_outlier_threshold, upper_outlier_threshold),mask, summed_log);
